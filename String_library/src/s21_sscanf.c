...
int s21_sscanf(const char *str, const char *format, ...) {
  ...
  stack *head = calloc(sizeof(stack), 1);
  head->next = s21_NULL;
  ...
  if (sconfig.string[sconfig.string_position] == '\0' &&
      sconfig.sscanf_counter == 0)
    sconfig.sscanf_counter = -1;
  free(sconfig.string);
  return sconfig.sscanf_counter;
}

char *deleting_extra_whitespaces(char *str) {
  char *fxdstr = calloc(sizeof(char), s21_strlen(str) + 1);
  char prev_ch = str[0];
  fxdstr[0] = str[0];
  s21_size_t j = 1;
...
stack *s21_flag_d(va_list *ss_arguments, scanfopt *sconfig, stack *tail) {
  sconfig->current_width = tail->width == 0 ? 2048 : tail->width;
  s21_what_sign(sconfig);
  char buffer[sconfig->current_width + 1];
  filling_buffer(buffer, sconfig, "decimal");
  s21_size_t value = s21_atoi_(buffer);
  if (!tail->skip) shove_and_check_int(ss_arguments, sconfig, tail, value);
  tail = s21_pop(tail);
  return tail;
}
...
stack *s21_flags_e(va_list *ss_arguments, stack *tail, scanfopt *sconfig) {
  ...
  if (!tail->skip) shove_and_check_float(ss_arguments, sconfig, tail, buffer);
  tail = s21_pop(tail);
  return tail;
}
...
stack *s21_flag_i(va_list *ss_arguments, stack *tail, scanfopt *sconfig) {
  ...
  if (!tail->skip)
    shove_and_check_int_for_i(ss_arguments, sconfig, tail, kostil);
  tail = s21_pop(tail);
  return tail;
}
stack *s21_flags_x(va_list *ss_arguments, stack *tail, scanfopt *sconfig) {
  ...
  s21_what_sign(sconfig);
  filling_buffer(buffer, sconfig, "hex");
  s21_size_t value = s21_convert_from_hex(buffer);
  if (!tail->skip) shove_and_check_uint(ss_arguments, sconfig, tail, value);
  tail = s21_pop(tail);
  return tail;
}
...
stack *s21_flag_o(va_list *ss_arguments, stack *tail, scanfopt *sconfig) {
  ...
  s21_what_sign(sconfig);
  filling_buffer(buffer, sconfig, "octal");
  s21_size_t value = s21_convert_from_octal(buffer);
  if (!tail->skip) shove_and_check_uint(ss_arguments, sconfig, tail, value);
  tail = s21_pop(tail);
  return tail;
}
...
void shove_and_check_int(va_list *ss_arguments, scanfopt *sconfig, stack *tail,
                         s21_size_t value) {
  void *ptr = va_arg(*ss_arguments, void *);
  if (tail->length == 'h') {
    short int tmp = (short)value * sconfig->current_sign;
    *(short *)ptr = (short)value * sconfig->current_sign;
    if (tmp == *(short *)ptr) sconfig->sscanf_counter++;
  } else if (tail->length == 'l') {
    long int tmp = (long)value * sconfig->current_sign;
    *(long int *)ptr = (long int)value * sconfig->current_sign;
    if (tmp == *(long *)ptr) sconfig->sscanf_counter++;
  } else {
    int tmp = (int)value * sconfig->current_sign;
    *(int *)ptr = (int)value * sconfig->current_sign;
    if (tmp == *(int *)ptr) sconfig->sscanf_counter++;
  }
}

void shove_and_check_int_for_i(va_list *ss_arguments, scanfopt *sconfig,
                               stack *tail, s21_size_t kostyl) {
  void *ptr = va_arg(*ss_arguments, void *);
  if (tail->length == 'h') {
    *(short int *)ptr = (short int)kostyl * sconfig->current_sign;
    if ((short int)kostyl * sconfig->current_sign == *(short int *)ptr)
      sconfig->sscanf_counter++;
  } else if (tail->length == 'l') {
    *(long int *)ptr = (long int)kostyl * sconfig->current_sign;
    if ((long int)kostyl * sconfig->current_sign == *(long int *)ptr)
      sconfig->sscanf_counter++;
  } else {
    *(int *)ptr = (int)kostyl * sconfig->current_sign;
    if ((int)kostyl * sconfig->current_sign == *(int *)ptr)
      sconfig->sscanf_counter++;
  }
}

void shove_and_check_uint(va_list *ss_arguments, scanfopt *sconfig, stack *tail,
                          s21_size_t value) {
  void *ptr = va_arg(*ss_arguments, void *);
  if (tail->length == 'h') {
    short unsigned int tmp = (short unsigned)value * sconfig->current_sign;
    *(short unsigned *)ptr = (short unsigned)value * sconfig->current_sign;
    if (tmp == *(short unsigned *)ptr) sconfig->sscanf_counter++;
  } else if (tail->length == 'l') {
    long unsigned int tmp = (long unsigned)value * sconfig->current_sign;
    *(long unsigned int *)ptr =
        (long unsigned int)value * sconfig->current_sign;
    if (tmp == *(long unsigned *)ptr) sconfig->sscanf_counter++;
  } else {
    unsigned int tmp = (unsigned int)value * sconfig->current_sign;
    *(unsigned int *)ptr = (unsigned int)value * sconfig->current_sign;
    if (tmp == *(unsigned int *)ptr) sconfig->sscanf_counter++;
  }
}

void shove_and_check_float(va_list *ss_arguments, scanfopt *sconfig,
                           stack *tail, char *buffer) {
  void *ptr = va_arg(*ss_arguments, void *);
  if (tail->length == 'L') {
    long double tmp = (long double)s21_atof(buffer) * sconfig->current_sign;
    *(long double *)ptr = (long double)s21_atof(buffer) * sconfig->current_sign;
    if (tmp == *(long double *)ptr) sconfig->sscanf_counter++;
  } else {
    float tmp = (float)s21_atof(buffer) * sconfig->current_sign;
    *(float *)ptr = (float)s21_atof(buffer) * sconfig->current_sign;
    if (tmp == *(float *)ptr) sconfig->sscanf_counter++;
  }
}
