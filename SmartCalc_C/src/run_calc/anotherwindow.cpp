...
AnotherWindow::AnotherWindow(const char *infix, double x1, double x2, double y1,
  ...
  Stack *input_expression = notation(infix);
  ...
  if (check.find('X') != -1 || check.find('x') != -1)
    input_expression = push(input_expression, 0, 'p');
  for (double X = x1; X <= x2 && status; X += rabbit) {
    if (input_expression->oper == 'p') input_expression->value = X;
    resx[i] = X;
    temp = calculate(input_expression);
